...
@if (session('error'))
<div class="flex items-center bg-red-100 text-red-700 px-4 py-3 rounded mb-4">
...
<div class="p-6 bg-white rounded shadow">
    ...
    <div class="grid md:grid-cols-2 gap-4 mb-6">
        <div class="bg-gray-50 p-4 rounded border">
            ...
            <form method="GET" action="{{ route('admin.penggunaan.index') }}" class="flex flex-col md:flex-row md:items-center gap-2">
                <div class="flex items-center gap-2 w-full">
                    <input type="text" name="nomor_kwh" placeholder="Nomor KWH" class="border rounded px-3 py-2 text-sm w-full" value="{{ request('nomor_kwh') }}">
                </div>
                <div class="flex gap-2">
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700">Terapkan</button>
                    <a href="{{ route('admin.penggunaan.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded text-sm hover:bg-gray-700">Reset</a>
                </div>
            </form>
        </div>
        ...
        <div class="bg-gray-50 p-4 rounded border flex flex-col justify-between">
            ...
            <button onclick="openCreateTagihanModal()" class="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700 w-fit">
        ...
        <table id="tagihanTable" class="min-w-full text-sm border">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="py-2 px-4 border">{{ \Carbon\Carbon::create()->month($penggunaan->bulan)->translatedFormat('F') }}</td>
                    ...
                    <td class="py-2 px-4 border">
                        <div class="flex gap-2 justify-center">
                            <a href="{{ route('admin.penggunaan.edit', $penggunaan->id_penggunaan) }}" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">
                                <i class="fa fa-pen"></i>
                            </a>
                            <form action="{{ route('admin.penggunaan.destroy', $penggunaan->id_penggunaan) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                @csrf
                                @method('DELETE')
