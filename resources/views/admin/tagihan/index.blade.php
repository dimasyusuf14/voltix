...
<div class="p-6">
    ...
    <div class="bg-white p-4 rounded shadow mb-6">
        <form method="GET" action="{{ route('admin.tagihan.index') }}" class="flex flex-col md:flex-row gap-4 md:items-end">
            <div class="flex-1">
                <label class="block text-sm font-medium mb-1">Status Pembayaran</label>
                <select name="status" class="w-full border px-3 py-2 rounded">
                    <option value="">-- Semua --</option>
                    <option value="Sudah Lunas" {{ request('status') == 'Sudah Lunas' ? 'selected' : '' }}>Sudah Lunas</option>
            ...
            <div class="flex-1">
                ...
                <input type="text" name="nomor_kwh" placeholder="Masukkan Nomor KWH" value="{{ request('nomor_kwh') }}" class="w-full border px-3 py-2 rounded">
            </div>
                    ...
                    <td class="px-4 py-2 border text-center">
                        <div class="flex justify-center gap-2">
                            ...
                            <a href="{{ route('admin.tagihan.print', $tagihan->id_tagihan) }}" target="_blank"
                                class="bg-indigo-500 text-white px-3 py-1 rounded hover:bg-indigo-600" title="Cetak Struk PDF">
                                <i class="fa fa-print"></i>
                            </a>
                            @endif
                            @if ($tagihan->status === 'Belum Lunas')
                            {{-- Tombol Bayar --}}
                            <a href="{{ route('admin.tagihan.bayar', $tagihan->id_tagihan) }}" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600" title="Bayar">
                                <i class="fa fa-wallet"></i>
                            </a>
                            @endif

                            {{-- Tombol Hapus --}}
                            <form action="{{ route('admin.tagihan.destroy', $tagihan->id_tagihan) }}" method="POST" onsubmit="return confirm('Hapus tagihan ini?')">
                                @csrf
                                @method('DELETE')
                                <button class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600" title="Hapus">
