...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @php
        // warna status & tombol
        $warna = $tagihan->status == 'Sudah Lunas' ? 'text-green-600' : 'text-red-600';
        $btnShow = $tagihan->status == 'Belum Lunas';
        @endphp

        <div class="bg-white shadow-sides-bottom rounded-xl p-6">
            <h2 class="text-lg font-bold mb-2">
                {{ \Illuminate\Support\Str::headline(\Carbon\Carbon::create($tagihan->tahun, $tagihan->bulan)->isoFormat('MMMM YYYY')) }}
            </h2>

            <p class="text-sm text-gray-500 font-medium">{{ $tagihan->no_invoice }}</p>
            <p class="text-sm text-gray-800 font-semibold">{{ $tagihan->pelanggan->nama_pelanggan }}</p>
            <p class="text-sm {{ $warna }} font-medium">{{ $tagihan->status }}</p>
            <p class="text-xs text-gray-500 mb-4">Status Pembayaran</p>

            <div class="border-t pt-3">
                <p class="text-sm text-gray-600">Total Tagihan</p>
                <p class="text-blue-600 text-lg font-bold">
                    Rp {{ number_format($tagihan->jumlah_meter * $tagihan->pelanggan->tarif->tarifperkwh, 0, ',', '.') }}
                </p>
                <p class="text-xs text-gray-500 mb-3">Sudah Termasuk Biaya Admin</p>

                @if ($btnShow)
                <a class="block w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-md text-center" href="{{ route('bayar.store', $tagihan->id_tagihan) }}">
                    Bayar Sekarang
                </a>
                @endif
            </div>
